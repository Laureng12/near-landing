      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; }
        ...
        .pricingTitle {
          ...
          letter-spacing: -0.02em; color: var(--plum-text, #2A0A16); margin: 0 0 16px;
        }
        .pricingTitle em { font-family: var(--font-serif, "Instrument Serif", Georgia, serif); font-style: italic; }
        .pricingSub {
          font-size: 17px; line-height: 1.55; color: var(--plum-text, #2A0A16);
          opacity: 0.7; max-width: 56ch; margin: 0 auto 32px;
        }
        .pricingToggle {
          display: inline-flex; background: rgba(26,14,31,0.06); border-radius: 999px; padding: 4px;
        }
        .toggleBtn {
          padding: 10px 24px; border-radius: 999px; border: 0; background: transparent;
          font-size: 14px; font-weight: 500; cursor: pointer; color: var(--plum-text, #2A0A16);
          font-family: inherit;
        }
        .toggleBtn.active { background: white; }
        .tiersGrid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; align-items: start; }
        .tierCard {
          background: white; border-radius: 24px; padding: 40px 32px;
          border: 1px solid rgba(26,14,31,0.06); color: var(--plum-text, #2A0A16); position: relative;
        }
        .tierCard.featured {
          background: var(--plum-base, #1A0E1F); color: var(--cream-text, #FFF0DB);
          border-color: var(--plum-base, #1A0E1F); transform: translateY(-8px);
        }
        .tierBadge {
        ...
        .tierPrice { font-size: 14px; margin: 0 0 8px; opacity: 0.6; display: flex; align-items: baseline; gap: 4px; }
        ...
        .tierList li { padding: 8px 0; font-size: 14px; line-height: 1.4; border-bottom: 1px solid rgba(26,14,31,0.06); }
        .featured .tierList li { border-color: rgba(255,240,219,0.08); }
        .tierCta {
          display: flex; align-items: center; justify-content: center; width: 100%;
          padding: 16px 28px; border-radius: 28px; font-size: 15px; font-weight: 500;
          text-decoration: none; font-family: inherit; text-align: center; box-sizing: border-box; cursor: pointer;
        }
        .tierCta.primary { background: var(--pink, #E56E8E); color: var(--cream-text, #FFF0DB); border: none; }
        .tierCta.ghost { background: transparent; color: var(--plum-text, #2A0A16); border: 1px solid rgba(26,14,31,0.2); }
        .featured .tierCta.ghost { color: var(--cream-text, #FFF0DB); border-color: rgba(255,240,219,0.3); }
        .pricingFootnote { text-align: center; font-size: 13px; line-height: 1.6; color: var(--plum-text, #2A0A16); opacity: 0.5; margin: 48px 0 0; }
        .dlBtn {
          display: inline-flex; align-items: center; padding: 10px 20px;
          background: var(--pink, #E56E8E); color: var(--cream-text, #FFF0DB);
          border-radius: 28px; font-size: 13px; font-weight: 500; text-decoration: none;
        }
